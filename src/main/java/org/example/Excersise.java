...
Wielkanoc jest tak zwanym świętem ruchomym - oznacza to, że każdego roku występuje w inny dzień. Definicja mówi,
iż przypada ona w pierwszą niedzielę po pierwszej pełni Księżyca przypadającej po równonocy wiosennej.
W 1991 roku Jean Meeus opublikował sposób na obliczanie daty Wielkanocy.
Aby obliczyć datę Wielkanocy:
·        Tworzymy zmienne a, b, c, d, e, f, g, h, i, k, l, m, p, zmienną przechowującą podany przez użytkownika rok,
         a także obliczony dzień i miesiąc
...
·        Za m podstaw zaokrąglony w dół wynik dzielenia przez 451 wyrażenia (a + 11 * h + 22 * l)
·        Za p podstaw resztę z dzielenia przez 31 wyrażenia (h + l – 7 * m + 114)
Aby wyznaczyć dzień Wielkanocy do p należy dodać 1, z kolei do wyznaczania miesiąca trzeba zaokrąglając w dół
podzielić (h + l – 7 * m + 114) przez 31. Jeżeli miesiąc wyszedł nam 4, wówczas Wielkanoc odbędzie się w kwietniu.
W każdym innym przypadku w marcu.
a)     Jakie działanie zwraca resztę z dzielenia? Czy wiesz jakim symbolem oznaczamy je w Javie? (modulo, procent)
b)     Jakiego typu podstawowego zmiennych użyjesz do implementacji? (int)
c)     W niektórych punktach algorytmu należy zaokrąglić wynik w dół z dzielenia. W jaki sposób możesz to zrobić?
        Czy potrzebujesz do tego specjalnej metody? (nie// Math, met. round)
d)     Przy pomocy jakiej klasy możesz pobrać od użytkownika rok? (scanner)
        Jakie znasz inne metody wprowadzania danych do aplikacji (niekoniecznie przy pomocy klawiatury)?
        (mozna podac z pliku albo argumentu programu)
